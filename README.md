...
## LaTeX で作成した理由
粉体工学会の Word 体裁を LuaLaTeX と LuaTeX-ja で忠実に再現するフォーマットを、有志で作成しました。これは単なるテンプレートではありません。論文執筆を道具の不安定さから解き放ちたい――その意思表示です。率直に申し上げると、Word は学術論文に向いていません。図表・数式・参考文献といった基盤で、あまりにも多くの時間と注意力をWordで失ってきました。

まず参照です。節や図表の位置を少し動かしただけでリンクが壊れ、図表番号が更新されない場面に何度も遭遇しました。共同編集では環境差が平然と紛れ込み、本文に「図 3」と書いてあるのに実体は図 2――そんな事故が起きます。版が進むほど整合性チェックというクリック作業が積み上がり、提出直前まで神経を削られます。これは執筆ではなく、復旧作業です。

数式も悩ましい点です。上付き・下付きが小さく、行間や字面のバランスが崩れ、複雑な分数や行列ほど読みにくくなります。ページをまたぐと式番号の位置が揺れ、主張の可読性が落ちます。参考文献も同様で、引用順を入れ替えたのに番号が揃わない、といった齟齬が平気で起きます。本来は仕様で機械的に保証されるべき部分が、人手の調整に委ねられていると感じます。

レイアウトも非決定的です。同じファイルでも端末が変わるだけで改ページや段組がずれ、見出しやキャプションが孤立します。図のアンカーは些細な編集で暴れ、本文との距離が勝手に開きます。形式が混在したファイルは保存や貼り付けのたびに挙動が重くなり、最悪は固まる／突然落ちることすらあります。有償ソフトでありながら、ここまで基本的な安定性を信頼できない状況に何度も悩まされました。

だからこそ、私は LaTeX を使います。余白・行送り・段間・キャプション・ラベル・フォントといった仕様をプリアンブルで固定します。本文は \label と \ref で参照を一貫させ、図表・式・節番号は移動や追加・削除に自動で追随します。引用もコマンドで一括管理し、本文と末尾の対応がずれません。数式は amsmath など成熟した仕組みにより字面と行間の均衡が保たれ、読みやすさと美しさが両立します。同じ入力なら、どこでビルドしても同じ PDF が得られます。

このフォーマットは、怒りから生まれました。論文の価値は内容にあり、体裁は機械が守るべきです。にもかかわらず、私は長らく、図表番号の手直しや参照の突合、崩れるレイアウトの鎮火に時間を奪われてきました。Word の運任せな組版から降り、体裁を仕様として固定し、LaTeX で共有する道を選びます。論文は内容で戦い、体裁は機械に任せる――その当たり前を取り戻したくて、このフォーマットを置きます。
